...
public class OrderDishesSaleStatisticsView extends SimpleForm implements Initializable {
    ...
    private void buildContent(double height) {
        // "编号", "菜品名称", "销售份数", "菜品总金额"
        tableView.getColumns().addAll(newCol("编号", rowIndex(), 100), //
                newCol("菜品名称", BO::getDishesName, 360),  //
                newCol("销售份数", BO::getCount, 150),  //
                newCol("菜品总金额", BO::getAllPrice, 150)  //
        );
    ...
    public static class BO {
        ...
        public static BO convertToBO(DishesSaleStatModel m) {
            BO bo = new BO();
            bo.setDishesName(m.getDishesName());
